...
class JiraClient:
    ...
    def get_boards_from_jira(self, start_index):
        ...
        return utils.execute_url(url=url, method="get", headers=self.header)[0]

    def get_sprints_from_jira(self, board_id, start_index):
        # Call JIRA API Endpoint to obtain all the sprints in the board
        url = BASE_URL + "/rest/agile/1.0/board/" + str(board_id) + "/sprint?maxResults=50&startAt=" + str(start_index)
    ...
    def get_subtask_details(self, sub_id):
        url = BASE_URL + "/rest/agile/1.0/issue/" + str(sub_id)
        return utils.execute_url(url=url, method="get", headers=self.header)[0]
